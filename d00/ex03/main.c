...
int main(void)
{
	...
	dict = dictInit(i);
	for (int x = 0; arts[x]; x++)
		dictInsert(dict, arts[x]->name, arts[x]);
	printf("name: %s price: %d 2ndprice: %d\n", dictSearch(dict, "Starry Night")->name, dictSearch(dict, "Starry Night")->price, searchPrice(dict, "Mona Lisa"));
